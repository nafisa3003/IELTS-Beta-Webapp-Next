...
import type { Question, AnswerOption, Passage } from "@/types/assessment";
...
export function QuestionRow({
  ...
  const attachedPassage = question.passageid ? passages.find((p) => p.passageid === question.passageid) : null;
      ...
      {editing ? (
        <form action={formAction} className="flex flex-wrap items-end gap-2">
          ...
          <select
            name="skill"
            value={skill}
            onChange={(e) => setSkill(e.target.value as Question["skill"])}
            className="rounded-md border border-mist px-2 py-1.5 text-sm"
          >
          ...
          {skill === "Reading" && (
            <select name="passageid" defaultValue={question.passageid ?? ""} className="rounded-md border border-mist px-2 py-1.5 text-sm">
          ...
          <p className="flex items-start gap-2 text-sm font-medium text-ink">
            ...
            <span>
              ...
              <span className="text-xs text-slate-soft">
                ({question.skill}, {question.marks}pt{attachedPassage ? `, Passage ${attachedPassage.order_index}` : ""})
              </span>
            </span>
          </p>
